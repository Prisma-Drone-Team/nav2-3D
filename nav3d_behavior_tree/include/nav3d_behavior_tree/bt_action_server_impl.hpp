...
#include "nav3d_util/copy_all_parameter_values.hpp"
  ...
  internal_error_msg_()
{
  ...
  if (!node->has_parameter("error_code_name_prefixes")) {
    ...
    if (value.get_type() == rclcpp::PARAMETER_NOT_SET) {
      ...
      rclcpp::Parameter error_code_name_prefixes_param(
        "error_code_name_prefixes", error_code_name_prefixes);
...
bool BtActionServer<ActionT>::on_configure()
{
  ...
  nav3d_util::copy_all_parameter_values(node, client_node_);
  ...
  bt_ = std::make_unique<nav3d_behavior_tree::BehaviorTreeEngine>(plugin_lib_names_, client_node_);
...
void BtActionServer<ActionT>::executeCallback()
{
  ...
  nav3d_behavior_tree::BtStatus rc = bt_->run(&tree_, on_loop, is_canceling, bt_loop_duration_);
  ...
  switch (rc) {
    case nav3d_behavior_tree::BtStatus::SUCCEEDED:
      action_server_->succeeded_current(result);
      RCLCPP_INFO(logger_, "Goal succeeded");
      break;

    case nav3d_behavior_tree::BtStatus::FAILED:
      action_server_->terminate_current(result);
      RCLCPP_ERROR(
    ...
    case nav3d_behavior_tree::BtStatus::CANCELED:
...
void BtActionServer<ActionT>::cleanErrorCodes()
{
  ...
  for (const auto & error_code_name_prefix : error_code_name_prefixes_) {
    name = error_code_name_prefix + "_error_code";
    blackboard_->set<unsigned short>(name, 0);
    name = error_code_name_prefix + "_error_msg";

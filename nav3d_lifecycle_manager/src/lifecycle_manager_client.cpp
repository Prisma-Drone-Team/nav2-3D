#include "nav3d_lifecycle_manager/lifecycle_manager_client.hpp"
  ...
  std::shared_ptr<rclcpp::Node> parent_node)
{
  ...
  manager_client_ = std::make_shared<nav3d_util::ServiceClient<ManageLifecycleNodes>>(
    manage_service_name_, node_);
  is_active_client_ = std::make_shared<nav3d_util::ServiceClient<std_srvs::srv::Trigger>>(
...
SystemStatus LifecycleManagerClient::is_active(const std::chrono::nanoseconds timeout)
{
  ...
  try {
    auto response = is_active_client_->invoke(request, timeout);
    return response->success ? SystemStatus::ACTIVE : SystemStatus::INACTIVE;
...
bool LifecycleManagerClient::callService(uint8_t command, const std::chrono::nanoseconds timeout)
{
  ...
  try {
    auto response = manager_client_->invoke(request, timeout);

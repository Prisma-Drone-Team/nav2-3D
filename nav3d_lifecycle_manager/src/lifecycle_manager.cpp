...
using nav3d_util::LifecycleServiceClient;
...
: Node("lifecycle_manager", options), diagnostics_updater_(this)
{
  ...
  init_timer_ = this->create_wall_timer(
    ...
    [this]() -> void {
      ...
      service_thread_ = std::make_unique<nav3d_util::NodeThread>(executor);
    });

  diagnostics_updater_.setHardwareID("Nav3D");
  diagnostics_updater_.add("Nav3D Health", this, &LifecycleManager::CreateActiveDiagnostic);
}
...
{
  if (system_active_) {
    stat.summary(diagnostic_msgs::msg::DiagnosticStatus::OK, "Nav3D is active");
  } else {
    stat.summary(diagnostic_msgs::msg::DiagnosticStatus::ERROR, "Nav3D is inactive");
  }
}
...
{
  RCLCPP_INFO(
    get_logger(), "Running Nav3D LifecycleManager rcl preshutdown (%s)",
    this->get_name());
...
void LifecycleManager::checkBondRespawnConnection()
{
  ...
  int live_servers = 0;
  const int max_live_servers = static_cast<int>(node_names_.size());
  for (auto & node_name : node_names_) {
    if (!rclcpp::ok()) {
      return;
    }
...
RCLCPP_COMPONENTS_REGISTER_NODE(nav3d_lifecycle_manager::LifecycleManager)